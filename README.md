...
This fork keeps the upstream disassembly and analysis intact, applies
the six-byte LN/ATN fix above in the source, and uses the rest of the
byte savings found in the ROM to build **two new versions** of BASIC
4r32, both still exactly 16K and both requiring a 65C02 (a Master 128,
or a B fitted with one):

- **`roms/Basic432_fast.rom`** targets **speed**: the same language,
  with fast paths for the things tight BASIC loops actually do —
  integer FOR loops run ~19% faster than stock 4r32, procedure calls
  ~36% faster. See [FAST.md](FAST.md).
- **`roms/Basic432_basicv.rom`** targets **features**: BASIC V-style
  structured programming — `WHILE`/`ENDWHILE` and multi-line
  `IF`/`ELSE`/`ENDIF` — at essentially stock speed. See
  [BASICV.md](BASICV.md).

The two feature sets do not fit in 16K together. Both are built from
the single conditional source `disassembly/Basic432.asm` (`make`,
`make check` to verify against the committed reference images), and
both are covered by the self-checking test suites in `tests/`, which
also documents how to run everything under jsbeeb.
